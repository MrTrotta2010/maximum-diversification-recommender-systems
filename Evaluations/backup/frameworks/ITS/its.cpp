	...
	if (strcmp(argv[2], "-a") == 0) {
	    ...
	    *output = (string)argv[5];
		...
		if (strcmp(argv[1], "PSO") == 0) {
			...
			if (argc != 8) {
				cout << "Guloso Naive: Argumentos insuficientes!\n";
				exit(1);
			}
	    	else {
			    *algoritmo = 4;
...
//		cout << "Teste " << i << "\n";
	...
	for (VectorOfUser::iterator itrUser = userPred.begin(); itrUser != userPred.end(); itrUser++) {
		...
		switch (algoritmo){
			...
			case 4:
				gbestUser[userId] = gulosoNaive(userId, userPred, hashFeature, testData, hashPred, hashSimilarity, itemRatings, args[0], args[1], iter_max, args[3], args[2]);
				break;
...
GBest gulosoNaive(int userId, VectorOfUser &userPred, VectorOfUser &hashFeature, HashOfHashes &testData, HashOfHashes &hashPred, HashOfHashes &hashSimilarity, HashOfHashes &itemRatings, int numPreds, float alfa, int iter_max, int swarmSize, int particleSize)
{
	...
	for (int i = 0; i < particleSize; i++) {
		...
		for (int j = 0; j < numPreds; j++) {
			...
			if (espacoDeBusca[j] != -1) {

				solucao->itens[i] = espacoDeBusca[j];
				solucao->posicoes[i] = j;
				
				float f = fitness(solucao, hashFeature, numPreds, alfa, i+1);
